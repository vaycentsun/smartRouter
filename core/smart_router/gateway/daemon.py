...
def _build_dashboard_app():
    """内联创建 Dashboard ASGI app

    直接内联而不引用 smart_router.web.server 子模块，
    解决旧版本 pip 安装的包中不存在该子目录的问题。
    """
    from .dashboard_api import build_dashboard_app

    # 优先使用当前源码目录中的 static 文件
    static_candidates = [
    ...
    return build_dashboard_app(static_dir)


def _write_dashboard_launcher_script() -> str:
    """生成临时 uvicorn 启动脚本（用于后台模式）

    后台模式通过 subprocess 启动全新的 Python 进程，无法共享当前进程的
    sys.path，因此将 app 创建逻辑写入临时脚本文件，由 uvicorn 直接加载。

    注意：脚本放在 /tmp/ 下，__file__ 指向临时路径，无法通过相对路径找到
    static 目录。因此将 daemon.py 所在的实际目录硬编码到脚本中。
    """
    import tempfile

    # daemon.py 所在目录（如 .../core/smart_router/gateway/）
    # static 目录在 .../core/smart_router/web/static/
    daemon_dir = str(Path(__file__).parent)

    script_content = f'''"""Dashboard launcher — generated by daemon.py"""
import sys
from pathlib import Path
import uvicorn

_DAEMON_DIR = Path(r"{daemon_dir}")

# 确保 smart_router 在 Python 路径中（开发模式需要）
_project_root = _DAEMON_DIR.parent.parent.parent
if str(_project_root) not in sys.path:
    sys.path.insert(0, str(_project_root))

from smart_router.gateway.dashboard_api import build_dashboard_app

static_dir = _DAEMON_DIR.parent / "web" / "static"
app = build_dashboard_app(static_dir if static_dir.exists() else None)
...
'''
    fd, path = tempfile.mkstemp(suffix="_dashboard_launcher.py")
...
def start_dashboard_daemon(
    ...
    if foreground:
        ...
        # 后台模式：生成临时启动脚本并直接执行
        # 脚本内部调用 uvicorn.run()，避免 uvicorn CLI 的模块导入限制
        python_exe = _get_python_executable()
        script_path = _write_dashboard_launcher_script()
        cmd = [python_exe, script_path, host, str(port)]
