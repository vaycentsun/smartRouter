...
class TestRoutingHistoryMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_middleware_records_success_status(self, mock_router):
        """验证成功请求记录 error_info='Success' 和 final_error_type='Success'"""
        ...
        assert records[0]["error_info"] == "Success"
        assert records[0]["final_error_type"] == "Success"

    @pytest.mark.asyncio
    async def test_middleware_records_success_after_fallback(self, mock_router):
        """验证 fallback 成功后 error_info 记录为 Success 并包含尝试次数"""
        from fastapi import Request
        from starlette.responses import Response
        from smart_router.gateway.server import SmartRouterMiddleware
        from smart_router.utils.request_routing_history import RequestRoutingHistory
        ...
        config = mock_router.sr_config
        config.models = {
            "model-a": MagicMock(provider="provider-a"),
            "model-b": MagicMock(provider="provider-b"),
            "model-c": MagicMock(provider="provider-c"),
        }
        config.is_model_available.return_value = True
        config.get_available_models.return_value = ["model-a", "model-b", "model-c"]
        ...
        async def mock_call_next(request):
            nonlocal call_count
            call_count += 1
            # 第一次返回 502，第二次返回 429，第三次成功
            if call_count == 1:
                return Response(
                    content=json.dumps({"error": "bad gateway"}).encode(),
                    status_code=502,
                    headers={"content-type": "application/json"},
                )
            if call_count == 2:
        ...
        record = records[0]
        assert record["status_code"] == 200
        assert record["selected_model"] == "model-c"
        assert record["error_info"] == "Success (after 2 fallback attempts)"
        assert record["final_error_type"] == "Success"
        assert len(record["retry_history"]) == 2

    @pytest.mark.asyncio
    async def test_middleware_records_failure_status(self, mock_router):
        """验证最终失败请求记录正确的 error_info 和 final_error_type"""
        from fastapi import Request
        from starlette.responses import Response
        from smart_router.gateway.server import SmartRouterMiddleware
        from smart_router.utils.request_routing_history import RequestRoutingHistory
        ...
        config.is_model_available.return_value = True
        config.get_available_models.return_value = ["model-a", "model-b"]
        config.routing.fallback.max_attempts = 2
        ...
        assert "model-b: HTTP 503" in record["error_info"]
        assert record["final_error_type"] == "ServiceUnavailable"