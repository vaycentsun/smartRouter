...
from ..selector.v3_selector import SelectionResult, V3ModelSelector
from ..utils.markers import MarkerResult, parse_markers
...
class SmartRouter(Router):
    ...
    def __init__(self, config: Config, config_dir: Optional[Path] = None, *args, **kwargs):
        ...
        self._rebuild_classifier(config)
        self._rebuild_selector(config)
        ...
        fallbacks = self._build_fallbacks(config)
    ...
    def _rebuild_classifier(self, config: Config) -> None:
        """从 V3 配置重建分类器"""
    ...
    def _rebuild_selector(self, config: Config) -> None:
        """重建模型选择器"""
        available_models = config.get_available_models()
        self.selector = V3ModelSelector(config=config, available_models=available_models)

    def _build_fallbacks(self, config: Config) -> list[dict]:
        """构建 LiteLLM fallbacks（只包含可用模型的 fallback 链）"""
        fallbacks = []
