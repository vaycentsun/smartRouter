      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[#1a1a2e]">
            ...
            >
              {t('REQUESTS')} <SortIcon column="request_count" sortKey={sortKey} sortAsc={sortAsc} />
            </th>
            <th
              className="text-right py-3 px-2 text-[10px] text-[#636366] font-mono uppercase tracking-widest cursor-pointer select-none"
              onClick={() => handleSort('prompt_tokens')}
            >
              {t('PROMPT')} <SortIcon column="prompt_tokens" sortKey={sortKey} sortAsc={sortAsc} />
            </th>
            <th
              className="text-right py-3 px-2 text-[10px] text-[#636366] font-mono uppercase tracking-widest cursor-pointer select-none"
              onClick={() => handleSort('completion_tokens')}
            >
              {t('COMPLETION')} <SortIcon column="completion_tokens" sortKey={sortKey} sortAsc={sortAsc} />
            </th>
            <th
              className="text-right py-3 px-2 text-[10px] text-[#636366] font-mono uppercase tracking-widest cursor-pointer select-none"
              onClick={() => handleSort('reasoning_tokens')}
            >
              {t('REASONING')} <SortIcon column="reasoning_tokens" sortKey={sortKey} sortAsc={sortAsc} />
            </th>
            <th
              className="text-right py-3 px-2 text-[10px] text-[#636366] font-mono uppercase tracking-widest cursor-pointer select-none"
              onClick={() => handleSort('cached_tokens')}
            >
              {t('CACHED')} <SortIcon column="cached_tokens" sortKey={sortKey} sortAsc={sortAsc} />
            </th>
            <th
              className="text-right py-3 px-2 text-[10px] text-[#636366] font-mono uppercase tracking-widest cursor-pointer select-none"
