...
**核心能力**:
- 统一入口：一个 API Key 管理多个服务商（OpenAI、Anthropic、Qwen、Kimi、MiniMax、GLM 等）
- 智能路由：自动识别任务类型（coding/writing/reasoning/brainstorming/chat）并选择最优模型
...
4. **验证脚本**: `verify.py` 用于快速功能验证（不依赖 pytest）
...
在 `selector/strategies.py` 的 `ModelSelector` 类中添加：
...
或在 `classifier/embedding.py` 的 `BUILTIN_TYPE_EXAMPLES` 中添加示例。
...
- `dev/README.md` - 项目说明
- `dev/QUICKSTART.md` - 5 分钟快速上手
- `dev/USAGE.md` - 详细使用指南
- `dev/templates/smart-router.yaml` - 配置模板（含详细注释）
- `specs/active/2026-04-18--smart-router.md` - 技术规格文档
